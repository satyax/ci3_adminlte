...
class Panel extends Admin_Controller {
	...
	public function admin_user_create()
	{
		...
		$form = $this->form_builder->create_form(null,true);  //create multipart form (bisa upload file)
      ...
      if (isset($files) && !empty($files) && $files["name"]!='') {
        $physical_location = $this->mSiteConfig['image_profile_real_folder'].$files["name"];
        $relative_file_location = $this->mSiteConfig['image_profile_folder'].$files["name"];
        if(file_exists($physical_location)) unlink($physical_location);
        rename($files['tmp_name'], $physical_location) or die("Failed to move file");
      }
			$additional_data = array(
				'first_name'	=> $this->input->post('first_name'),
				'last_name'		=> $this->input->post('last_name'),
        'profile_picture' => $files["name"],
			);
	...
	{
		// Update Info form
		$form1 = $this->form_builder->create_form('admin/panel/account_update_info',true);  //create multipart form (bisa upload file)
    ...
    if (isset($files) && !empty($files) && $files["name"]!='') {
      ...
      if(file_exists($physical_location)) unlink($physical_location);
      rename($files['tmp_name'], $physical_location) or die("Failed to move file");
      $data['profile_picture'] = $files["name"];
    }
