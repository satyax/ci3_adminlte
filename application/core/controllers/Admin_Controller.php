...
class Admin_Controller extends MY_Controller {
    ...
    $this->mViewData['image_folder'] = $this->mSiteConfig['image_folder'];
	...
	protected function generate_crud($table, $subject = '',$arrWhere = null)
	{
		// create CRUD object
    ...
    //wawan tambahin ini disini
    if (!is_null($arrWhere)) $crud->where($arrWhere);
  ...
  protected function render_crud_grid_only($output_view_to_variable = false)
  {
    // logic specific for Grocery CRUD only
    $crud_obj_name = strtolower(get_class($this->mCrud));
    if ($crud_obj_name==='grocery_crud')
    ...
    // display view
    $this->mViewData['crud_output'] = $crud_data->output;
    if ($output_view_to_variable) {
      return $this->render('crud',$output_view_to_variable);
    } else {
      $this->render('crud',$output_view_to_variable);
    }
  }
